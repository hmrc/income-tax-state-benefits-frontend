...
class AmountController @Inject()(actionsProvider: ActionsProvider,
  ...
  def show(taxYear: Int, sessionDataId: UUID, benefitTypeUrl: String): Action[AnyContent] =
    actionsProvider.userSessionDataFor(taxYear, sessionDataId) { implicit request =>
  ...
  private def getRedirectCall(taxYear: Int,
    ...
    if (userData.isFinished) ReviewClaimController.show(taxYear, sessionDataId, benefitTypeUrl) else TaxTakenOffController.show(taxYear, sessionDataId, benefitTypeUrl)
  }
}
