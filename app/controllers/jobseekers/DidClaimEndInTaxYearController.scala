...
class DidClaimEndInTaxYearController @Inject()(actionsProvider: ActionsProvider,
  ...
  def show(taxYear: Int,
    ...
    Ok(pageView(DidClaimEndInTaxYearPage(taxYear, request.stateBenefitsUserData, formsProvider.endDateYesNoForm(taxYear, benefitType), benefitType)))
  }
  ...
  def submit(taxYear: Int,
    ...
    formsProvider.endDateYesNoForm(taxYear, benefitType).bindFromRequest().fold(
      formWithErrors => {
        Future.successful(BadRequest(pageView(DidClaimEndInTaxYearPage(taxYear, request.stateBenefitsUserData, formWithErrors, benefitType))))
      },
