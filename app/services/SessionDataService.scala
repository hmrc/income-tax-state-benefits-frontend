...
@Singleton
class SessionDataService @Inject()(sessionDataConnector: SessionDataConnector, config: AppConfig)
  ...
  private[services] def getFallbackSessionData[A](sessionId: String)(implicit request: Request[A]): Option[UserSessionData] =
    (
  ...
  private[services] def getSessionDataFromSessionStore()(implicit hc:HeaderCarrier): Future[Option[UserSessionData]] =
    if (config.sessionCookieServiceEnabled) {
      sessionDataConnector.getSessionData.map {
        ...
        case Left(err) =>
          logger.error(
            s"Request to retrieve session data failed with error status: ${err.status} and error body: ${err.body}. Returning None")
          None
      }
    } else {
