...
class SectionCompletedServiceISpec extends IntegrationTest {
  ...
  "SectionCompletedService" should {
    ...
    "return the correct journey answers from the connector" in {
      stubFor(get(urlEqualTo(s"/income-tax-state-benefits/journey-answers/$journeyName/$taxYear"))
      ...
      verify(getRequestedFor(urlEqualTo(s"/income-tax-state-benefits/journey-answers/$journeyName/$taxYear"))
        .withHeader("MTDITID", equalTo(mtdItId)))
    }

    "set journey answers successfully via the connector" in {
      stubFor(post(urlEqualTo("/income-tax-state-benefits/journey-answers"))
        .withHeader("MTDITID", equalTo(mtdItId))
      ...
      verify(postRequestedFor(urlEqualTo("/income-tax-state-benefits/journey-answers"))
        .withHeader("MTDITID", equalTo(mtdItId))
        .withRequestBody(equalTo(Json.toJson(journeyAnswers).toString())))
    }

    "keep journey alive successfully via the connector" in {
      stubFor(post(urlEqualTo(s"/income-tax-state-benefits/journey-answers/keep-alive/$journeyName/$taxYear"))
        .withHeader("MTDITID", equalTo(mtdItId))
        .willReturn(aResponse().withStatus(NO_CONTENT))
      )
      ...
      verify(postRequestedFor(urlEqualTo(s"/income-tax-state-benefits/journey-answers/keep-alive/$journeyName/$taxYear"))
        .withHeader("MTDITID", equalTo(mtdItId)))
    }
  }
}
