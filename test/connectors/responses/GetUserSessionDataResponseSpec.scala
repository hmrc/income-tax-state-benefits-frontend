...
import connectors.responses.GetUserSessionDataResponse.getUserSessionDataResponseReads
...
class GetUserSessionDataResponseSpec extends UnitTest {

  private val anyHeaders: Map[String, Seq[String]] = Map.empty
  private val anyMethod: String = "POST"
  private val anyUrl = "/any-url"

  private val underTest = getUserSessionDataResponseReads

  "GetUserSessionDataResponse" should {
    "convert JsValue to GetUserSessionDataResponse" when {
      "status is OK and valid jsValue" in {
        ...
        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe GetUserSessionDataResponse(
          httpResponse,
          Right(aStateBenefitsUserData)
        )
      }
