...
trait ViewHelper {
  ...
  def linkCheck(text: String,
                selector: String,
                href: String,
                hiddenText: Option[String] = None,
                hiddenTextSelector: Option[String] = None,
                isExactUrlMatch: Boolean = true,
                additionalTestText: String = "")(implicit document: Document): Unit = {
    ...
    s"has a link to change the language to $otherLanguage" which {
      ...
      s"has a link to change the language" in {
        document.select(".hmrc-language-select__list-item > a").attr("href") shouldBe
          s"/update-and-submit-income-tax-return/state-benefits/language/${linkLanguage(otherLanguage).toLowerCase}"
      }
    }
  }
}
