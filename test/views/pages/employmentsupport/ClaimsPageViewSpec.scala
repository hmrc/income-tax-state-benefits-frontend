  ...
  userScenarios.foreach { userScenario =>
    ...
    s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" should {
      "render page without any claims" which {
        ...
        val pageModel = aClaimsPage.copy(benefitType = EmploymentSupportAllowance, summaryListDataRows = Seq.empty)
        ...
        formPostLinkCheck(UserSessionDataController.create(taxYearEOY, EmploymentSupportAllowance).url, Selectors.addMissingClaimFormSelector)
        buttonCheck(expectedButtonText, Selectors.buttonSelector, Some(SectionCompletedQuestionController.show(taxYearEOY, EmploymentSupportAllowance).url))
      }

      "render page with claims" which {
        implicit val userPriorDataRequest: UserPriorDataRequest[AnyContent] = getUserPriorDataRequest(userScenario.isAgent)
        implicit val messages: Messages = getMessages(userScenario.isWelsh)

        val pageModel = aClaimsPage.copy(benefitType = EmploymentSupportAllowance, summaryListDataRows = Seq(
        ...
        linkCheck(userScenario.commonExpectedResults.expectedViewLinkText, Selectors.summaryListRowViewLinkSelector(1), UserSessionDataController.loadToSession(taxYearEOY,
          EmploymentSupportAllowance, aBenefitSummaryListRowData.benefitId).url, Some(expectedViewLinkHiddenText), Some(Selectors.summaryListRowViewLinkHiddenTextSelector(1)))
      ...
      "render the page for an inYear tax claim" which {
        implicit val userPriorDataRequest: UserPriorDataRequest[AnyContent] = getUserPriorDataRequest(userScenario.isAgent)
        implicit val messages: Messages = getMessages(userScenario.isWelsh)
        val pageModel = aClaimsPage.copy(benefitType = EmploymentSupportAllowance, taxYear = taxYear, isInYear = true)
        ...
        linkCheck(userScenario.commonExpectedResults.expectedViewLinkText,
          Selectors.summaryListRowViewLinkSelector(1, isInYear = true), UserSessionDataController.loadToSession(taxYear, EmploymentSupportAllowance, aBenefitSummaryListRowData.benefitId).url,
        ...
        buttonCheck(expectedButtonText, Selectors.buttonSelector, Some(SectionCompletedQuestionController.show(taxYear, EmploymentSupportAllowance).url))
      }
    }
  }
}
