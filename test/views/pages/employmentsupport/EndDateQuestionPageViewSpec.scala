  ...
  userScenarios.foreach { userScenario =>
    ...
    s"language is ${welshTest(userScenario.isWelsh)}" should {
      "render page with empty form" which {
        ...
        formPostLinkCheck(EndDateQuestionController.submit(taxYearEOY, EmploymentSupportAllowance, pageModel.sessionDataId).url, Selectors.continueButtonFormSelector)
      ...
      "render page with empty selection error" which {
        ...
        val pageModel = aEndDateQuestionPage.copy(benefitType = EmploymentSupportAllowance, form = aEndDateQuestionPage.form.bind(Map(YesNoForm.yesNo -> "")))
        implicit val document: Document = Jsoup.parse(underTest(pageModel).body)
