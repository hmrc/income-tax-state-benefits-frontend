...
class SectionCompletedQuestionPageViewSpec extends ViewUnitTest {
  ...
  userScenarios.foreach { userScenario =>
    ...
    s"language is ${welshTest(userScenario.isWelsh)}" should {
      val pageModel = aSectionCompletedQuestionPage.copy(benefitType = EmploymentSupportAllowance)
      "render page with empty form" which {
        ...
        formPostLinkCheck(SectionCompletedQuestionController.submit(taxYearEOY, EmploymentSupportAllowance).url, Selectors.continueButtonFormSelector)
      ...
      "render page with empty selection error" which {
        ...
        val page = pageModel.copy(form = pageModel.form.bind(Map(YesNoForm.yesNo -> "")))
        implicit val document: Document = Jsoup.parse(underTest(page).body)
